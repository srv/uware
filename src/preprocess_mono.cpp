...
namespace uware
{
        ...
        const cola2_msgs::NavSts::ConstPtr& navstatus_msg)
    {
        ...
        double FOV_y = 26.84 * ((2*M_PI)/360.0); //// remember that the field of view does not change with the image resolution. 
                                                ////  dimensions of FOV in x and y in metric units, depending on the flying height   
        ...
        if (!getOdom2CameraTf(params_.base_link_frame_id, img_msg->header.frame_id, odom2camera_))
        {
        ...
        int binning_x = info_msg->binning_x;
        int binning_y = info_msg->binning_y;
        const cv::Mat P(3,4, CV_64FC1, const_cast<double*>(info_msg->P.data()));
        ...
        if (dist > params_.store_distance)
        {
            if (altitud < params_.max_altitude){
                ...
                // Stamp
                double stamp = img_msg->header.stamp.toSec();
                ...
                cv::Mat img;
                mono_imgMsgToMat(*img_msg, img);
                mono_storeImages(img);
    ...
    void PreProcess::mono_storeImages(cv::Mat img)
    {
        cv::imwrite(params_.outdir + "/" + IMG_DIR + "/" + Utils::id2str(id_) + ".jpg", img);
    }


    bool PreProcess::mono_imgMsgToMat(sensor_msgs::Image img_msg,
                                        cv::Mat &img)
    {
        ...
        try
        {
            img_ptr = cv_bridge::toCvCopy(img_msg, enc::BGR8);
    ...
    tf::Transform PreProcess::data2Tf(double  W, double  Y, double roll1, double pitch1, double yaw1){
        ...
        tf::Transform t(tf_orientation, tf_translation);
        return t; 
    }
